...
export async function initializeDB(): Promise<IDBPDatabase<BOSIndexDB>> {
    const db = await openDB<BOSIndexDB>(BOS_INDEX_DB, BOS_INDEX_DB_VERSION, {
        upgrade(db) {
            const store = db.createObjectStore(COMPONENTS_CACHE_STORE_NAME, { keyPath: 'key' });
...
export async function addComponentToCache(db: IDBPDatabase<BOSIndexDB>, data: ComponentCacheRecord): Promise<void> {
    await db.put(COMPONENTS_CACHE_STORE_NAME, data)
}

export function retrieveComponentFromCache(db: IDBPDatabase<BOSIndexDB>, componentName: string): Promise<ComponentCacheRecord | undefined> {
