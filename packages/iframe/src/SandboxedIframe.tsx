    ...
    <html>
      ...
      <body>
        ...
        <script type="module">
          ...
          const { h: createElement, render, options } = window.preact;
          
          // register handler executed upon vnode render
          options.diffed = (vnode) => {
            // this handler will fire for every descendant node rendered,
            // but the '_' component appears to always be the root Component
            if (vnode.type?.name === '_') {
              dispatchRenderEvent(vnode);
            }
          };
