 ...
 * @param createElement Preact's createElement method
 * @param postCallbackInvocationMessage Request invocation on external Widget via window.postMessage
  ...
  return function processEvent(event: PostMessageEvent) {
    ...
    switch (event.data.type) {
        case 'widget.callbackInvocation': {
          ...
          result = applyRecursivelyToComponents(result, (n: any) => serializeNode({ createElement, node: n, callbacks, parentId: method, childWidgets: [], index: 0 }))
